...
def main(args: argparse.Namespace) -> None:
    ...
    if "qwen3" not in LLMTemplate.list_available():
        qwen3_template = LLMTemplate(
            model_type="qwen3",
            kv_layers_name=["*k_proj", "*v_proj"],
            q_layer_name="*q_proj",
            exclude_layers_name=["lm_head"],
            awq_config=get_algo.get_algo_qwen3_config("awq", "qwen3"),
        )
        # register
        LLMTemplate.register_template(qwen3_template)
   
    # verfiy
    print("Available templates:", LLMTemplate.list_available())
    tt = LLMTemplate.get("qwen3")
    print("qwen3 template:", tt.kv_layers_name, tt.exclude_layers_name)
    ...
    if not args.skip_quantization:
        ...
        print("model_c_t",model_config_type)
